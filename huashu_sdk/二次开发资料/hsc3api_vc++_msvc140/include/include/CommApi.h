...
public:
    ...
    */
    ...
    /** 
    *   @brief  配置心跳功能
    *   @details    若启用心跳功能，在TCP通信已连接后，将自动发送心跳帧。
    *               注：CommApi构造时默认启用心跳功能。建议在调用NetConnect接口前进行配置。
    *   @param  en  是否启用
    */
    void setUseHeartBeat(bool en);
    ...
    */
    ...
    /** 
    *   @brief  执行命令
    *   @details    注：使用该接口会阻塞调用线程，直至成功/超时（约3s）失败/其它原因导致的失败。
    ...
    */
    ...
    /**
    *   @brief  执行命令
    *   @details    注：使用该接口会阻塞调用线程，直至成功/超时（约3~6s）失败/其它原因导致的失败。
    ...
    *   @see        setUseUdp
    */
    HMCErrCode getUdpMsg(UdpDataType type, std::string & strMsg);
    ...
    *   @param  iport       端口号
    */
    void configFtp(const std::string & strUser, const std::string & strPasswd , int iport);
...
/** 
*   @brief  配置扫描目标网络
*   @details可配置扫描的目标网络，可不调用（不调用则选择默认配置）。注：正在扫描时调用会失败。
*   @param  strAddr 网络地址（例如“10.0.0.0”）
*   @param  strMask 网络掩码（例如“255.0.0.0”）
*   @param  uPort   端口
*   @return 错误码（0x0成功）
*   @see        scanHsc3Devices
*/
DLL_EXPORT HMCErrCode configScanTargetNetwork(const std::string strAddr, const std::string strMask, uint16_t uPort);
