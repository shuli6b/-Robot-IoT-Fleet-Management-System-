...
enum AutoState
{
    ...
    AUTO_STATE_LOADING,     ///< <em>8</em> - 加载中
    AUTO_STATE_MAX,
};
...
{
    GROUP_STATUS_MIN = -1,
    GROUP_STATUS_POWER_OFF = 0,       ///< <em>0</em> - 掉电状态
    GROUP_STATUS_ERROR_STOP = 1,      ///< <em>1</em> - 错误停机
    GROUP_STATUS_STOP = 2,            ///< <em>2</em> - 停止状态
    GROUP_STATUS_STAND_STILL = 3,     ///< <em>3</em> - 使能状态
    GROUP_STATUS_DISC_MOVING = 4,     ///< <em>4</em> - 离散运动状态
    GROUP_STATUS_COUS_MOVING = 5,     ///< <em>5</em> - 连续运动状态
    GROUP_STATUS_SYNC_MOVING = 6,     ///< <em>6</em> - 同步运动状态
    GROUP_STATUS_HOME = 7,            ///< <em>7</em> - 回零
    GROUP_STATUS_EXT_ERROR = 8,       ///< <em>8</em> - 外部错误
    GROUP_STATUS_MAX,
};
...
struct LocPos
{
    ...
    int32_t config;     ///< config 【注意】参考机器人程序点位定义中的CONFIG定义, 可通过getConfig(int8_t gpId, int32_t & config)接口获取
    std::vector<double> vecPos; ///< 点数据
};
...
struct GroupData
{
    ...
    int8_t ufNum;       ///< 工件号
    int8_t utNum;       ///< 工具号
    int32_t config;     ///< 形态 【注意】参考机器人程序点位定义中的CONFIG定义, 可通过getConfig(int8_t gpId, int32_t & config)接口获取
...
struct ProgInfo
{
    ...
    int32_t line_no;        ///< 行号
    std::string back_prog;
    int32_t line_of_back;
...
/**
*   @struct     ServoParaInfo
*   @brief      伺服参数信息
*/
struct ServoParaInfo
{
    std::string name;               ///< 参数名
    std::string chName;             ///< 中文名
    std::string unitName;           ///< 单位
    double minValue;                ///< 最小值
    double maxValue;                ///< 最大值
    double value;                   ///< 当前值
};
