        ...
        */
        ...
        public:
            ...
            */
			...
			/**
            * @brief   设置执行模式
            * @param   mode        模式( 0 : NORMAL(普通模式) ; 1: OPT(优化模式))
			* @details 注：接口非立刻生效，需通过状态获取接口判断。
            */
			Hsc3::Comm::HMCErrCode setMode(const int & mode);

            /**
            * @brief   加载程序
            * @param   path        路径（不包含文件名）一般路径：/usr/codesys/hsc3_app/script/
            * @details 注：接口非立刻生效，需通过状态获取接口判断。
            * @param   fileName    主程序文件名（文件名如在路径下的子目录，则需要包含子目录）
            * @see     getProgInfo
            */
            ...
            /**
            * @brief   启动运行
            * @details 注：接口非立刻生效，需通过状态获取接口判断。(在程序状态为就绪或者暂停时，可调用)
            ...
            */
			...
			/**
            * @brief   程序内跳转
			* @detail	(在程序状态为就绪或者暂停时，可调用)
            * @param   fileName    主程序文件名
			* @param   lineNum		行号
            * @see     getProgInfo
            */
	        Hsc3::Comm::HMCErrCode jump(const std::string & fileName, int & lineNum);

			/**
            * @brief   回退
            * @param   fileName    主程序文件名
            */
	        Hsc3::Comm::HMCErrCode back(const std::string & fileName);
            ...
            * @brief   程序是否已加载
            * @param   fileName    主程序文件名
            * @param[out]  isLoaded     是否加载
            */
	        Hsc3::Comm::HMCErrCode isLoaded(const std::string & fileName, bool & isLoaded);
